 ...
 * Plugin URI: https://example.com/live-event-manager
...
if (!class_exists('LEM_Object_Cache_Adapter')) {
    class LEM_Object_Cache_Adapter {
        private $group = 'lem';
        ...
        public function del($key) {
            if (is_array($key)) {
                ...
                foreach ($key as $single) {
                    if (wp_cache_delete($single, $this->group)) {
                        $count++;
                    }
                }
                return $count;
            }
            return wp_cache_delete($key, $this->group) ? 1 : 0;
        }
        ...
        public function info() {
            return array(
                'redis_mode' => 'object-cache',
                'object_cache_client' => (defined('WP_REDIS_CLIENT') ? WP_REDIS_CLIENT : 'wp_cache')
            );
        }
...
class LiveEventManager {
    ...
    // Static connection pool for Redis (reuse connections)
    private static $redis_instance = null;
    
    // In-memory cache for current request (reduces Redis calls)
    private static $memory_cache = array();
    
    public function __construct() {
        // Initialize device identification service
        require_once plugin_dir_path(__FILE__) . 'device-identification-service.php';
        ...
        add_action('wp_ajax_nopriv_lem_generate_jwt', array($this, 'ajax_generate_jwt'));
        ...
        add_action('wp_ajax_lem_refresh_jwt', array($this, 'ajax_refresh_jwt'));
        add_action('wp_ajax_nopriv_lem_refresh_jwt', array($this, 'ajax_refresh_jwt'));
    ...
    public function init() {
        ...
        // Ensure rewrite rules are added
        ...
        add_filter( 'theme_page_templates', function( $templates ) {
            $templates['page-events.php'] = __( 'Events (lem_events)', 'my-plugin' );
            return $templates;
        });
    ...
    private function ensure_tables_exist() {
        ...
        $jwts_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = %s AND table_name = %s", DB_NAME, $table_jwts));
    ...
    private function create_tables() {
        ...
        if ($result !== false) {
            // Add indexes and constraints after table creation
            $indexes_sql = array(
    ...
    public function render_event_meta_box($post) {
        ...
        if ($is_free === false || $is_free === '') {
            $is_free = 'free';
        }
        
        // Fetch available streams from both providers
        ...
        if ($credentials) {
            ...
            $streams_result = $this->list_live_streams($request);
        ...
        <table class="form-table">
            <tr>
                <th><label for="lem_stream_selector">Select Stream</label></th>
                <td>
                    <select id="lem_stream_selector" class="regular-text" style="width: 100%;">
                        <option value="">-- Select a stream to auto-fill details --</option>
                        <?php foreach ($available_streams as $stream): ?>
                            ...
                            $playback_id_value = 'N/A';
                            if (!empty($playback_ids)) {
                                if (isset($playback_ids[0]['id'])) {
                                    $playback_id_value = $playback_ids[0]['id'];
                                } elseif (is_string($playback_ids[0])) {
                                    $playback_id_value = $playback_ids[0];
                                }
                            }
                            // Check if this stream matches the current event's settings
                            $is_selected = ($live_stream_id === $stream_id) && ($stream_provider === $provider);
                            ?>
                            <option value="<?php echo esc_attr($stream_id); ?>" 
                                    data-playback-id="<?php echo esc_attr($playback_id_value); ?>"
                                    data-stream-id="<?php echo esc_attr($stream_id); ?>"
                                    data-provider="<?php echo esc_attr($provider); ?>"
                                    <?php selected($is_selected); ?>>
                                [<?php echo esc_html(strtoupper($provider)); ?>] <?php echo esc_html($stream_name); ?> 
                                <?php if ($playback_id_value !== 'N/A'): ?>
                                    (Playback: <?php echo esc_html(substr($playback_id_value, 0, 20)); ?>...)
                                <?php endif; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <p class="description">Select a stream to automatically populate the Playback ID and Live Stream ID fields below.</p>
                </td>
            </tr>
            <tr>
                <th><label for="lem_playback_id">Playback ID *</label></th>
                <td>
                    <input type="text" id="lem_playback_id" name="lem_playback_id" value="<?php echo esc_attr($playback_id); ?>" class="regular-text" required>
                    <p class="description">Mux Playback ID for the video player</p>
                </td>
            </tr>
            <tr>
                <th><label for="lem_live_stream_id">Live Stream ID</label></th>
                <td>
                    <input type="text" id="lem_live_stream_id" name="lem_live_stream_id" value="<?php echo esc_attr($live_stream_id); ?>" class="regular-text">
                    <input type="hidden" id="lem_stream_provider" name="lem_stream_provider" value="<?php echo esc_attr($stream_provider); ?>">
                    <p class="description">Mux Live Stream ID for publishing</p>
                    <p>
                        <a href="<?php echo admin_url('admin.php?page=live-event-manager-stream-setup'); ?>" target="_blank" class="button button-small">Get Publish Info & Setup Stream</a>
                        <a href="<?php echo admin_url('edit.php?post_type=lem_event&page=live-event-manager-publisher'); ?>" target="_blank" class="button button-small">Open Publisher</a>
                    </p>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <select id="lem_playback_restriction_id" name="lem_playback_restriction_id" data-current-value="<?php echo esc_attr($playback_restriction_id); ?>">
                    ...
                    <button type="button" id="lem-refresh-restrictions" class="button button-small">Refresh Restrictions</button>
                    <p class="description">Select a playback restriction or leave empty</p>
                </td>
            </tr>
            <tr>
                <th><label for="lem_event_date">Event Date & Time *</label></th>
                <td>
                    <input type="datetime-local" id="lem_event_date" name="lem_event_date" value="<?php echo esc_attr($event_date); ?>" required>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <label>
                        <input type="radio" name="lem_is_free" value="free" <?php checked($is_free, 'free'); ?>> Free Event
                    </label>
                    <br>
                    <label>
                        <input type="radio" name="lem_is_free" value="paid" <?php checked($is_free, 'paid'); ?>> Paid Event
                    </label>
                </td>
            </tr>
            <tr id="lem-pricing-fields" style="display: <?php echo $is_free === 'paid' ? 'table-row' : 'none'; ?>;">
                ...
                <td>
                    <input type="text" id="lem_price_id" name="lem_price_id" value="<?php echo esc_attr($price_id); ?>" class="regular-text">
                    <p class="description">Create a price in Stripe Dashboard and use the price ID here</p>
                </td>
            </tr>
            <tr id="lem-display-price-field" style="display: <?php echo $is_free === 'paid' ? 'table-row' : 'none'; ?>;">
                ...
                <td>
                    <input type="text" id="lem_display_price" name="lem_display_price" value="<?php echo esc_attr(get_post_meta($post->ID, '_lem_display_price', true)); ?>" class="regular-text" placeholder="e.g., $19.99">
                    <p class="description">The price to display on the event page (e.g., $19.99, £15.00, €25)</p>
                </td>
            </tr>
            <tr>
                <th><label for="lem_excerpt">Event Excerpt</label></th>
                <td>
                    <textarea id="lem_excerpt" name="lem_excerpt" rows="4" cols="50" class="large-text" placeholder="Brief description of the event for the listing page..."><?php echo esc_textarea(get_post_meta($post->ID, '_lem_excerpt', true)); ?></textarea>
                    <p class="description">A brief description that will appear on the events listing page (recommended: 150 characters or less)</p>
                </td>
            </tr>
            <!-- Status removed - now using WordPress native Draft/Published status in sidebar -->
        </table>
        
        <script>
        jQuery(document).ready(function($) {
            // Function to show/hide pricing fields
            function updatePricingFields() {
                var isPaid = $('input[name="lem_is_free"]:checked').val() === 'paid';
                $('#lem-pricing-fields, #lem-display-price-field').toggle(isPaid);
            }
            
            // Set initial state on page load
            updatePricingFields();
            
            // Show/hide pricing fields when event type changes
            $('input[name="lem_is_free"]').on('change', function() {
                updatePricingFields();
            });
        });
        </script>
        ...
        jQuery(document).ready(function($) {
            ...
            $('input[name="lem_is_free"]').on('change', function() {
                ...
                $('#lem-pricing-fields').toggle(!isFree);
            });
            
            // Load restrictions on page load (only if dropdown exists)
                ...
                $.post(lem_ajax.ajax_url, {
                    ...
                    if (response.success) {
                        ...
                        if (showSuccessMessage) {
                            showNotification('Restrictions loaded successfully', 'success');
                        }
                    }
                }).always(function() {
    ...
    public function load_event_template($template) {
        ...
        if ($post && $post->post_type === 'lem_event') {
            $event_template = LEM_PLUGIN_DIR . 'templates/single-event.php';
    ...
    public function load_event_archive_template($template) {
        if (is_post_type_archive('lem_event')) {
            $archive_template = LEM_PLUGIN_DIR . 'templates/page-events.php';
    ...
    public function add_rewrite_rules() {
        ...
        // Force flush rewrite rules to ensure they take effect
        flush_rewrite_rules();
        
        $this->debug_log('Rewrite rules added and flushed');
    }
    ...
    public function load_additional_templates($template) {
        ...
        if (get_query_var('lem_events_page')) {
            $events_template = LEM_PLUGIN_DIR . 'templates/page-events.php';
    ...
    private function clear_session_cookie() {
        if (isset($_COOKIE['lem_session_id'])) {
            setcookie('lem_session_id', '', time() - 3600, '/', '', is_ssl(), true);
            ...
            if (!empty($magic_token)) {
                ...
                if (!headers_sent()) {
                    if (!empty($result['valid'])) {
                        ...
                        if (!empty($session_id)) {
                            setcookie('lem_session_id', $session_id, time() + DAY_IN_SECONDS, '/', '', is_ssl(), true);
    ...
    // Session management methods with batch operations
    public function create_session($jti, $event_id, $email) {
        ...
        if ($redis) {
            ...
            // Use pipeline to batch all writes (single network round-trip)
            $use_pipeline = method_exists($redis, 'pipeline') && !($redis instanceof LEM_Object_Cache_Adapter);
            
            if ($use_pipeline) {
                $pipe = $redis->pipeline();
                $pipe->setex("session:{$session_id}", $ttl, $session_json);
                $pipe->setex("jti_session:{$jti}", $ttl, $session_id);
                $pipe->setex("active_session:{$event_id}:{$email_hash}", $ttl, $session_id);
                $pipe->execute();
            } else {
                // Fallback to individual calls
                $redis->setex("session:{$session_id}", $ttl, $session_json);
                $redis->setex("jti_session:{$jti}", $ttl, $session_id);
                $redis->setex("active_session:{$event_id}:{$email_hash}", $ttl, $session_id);
            }
    ...
    public function validate_session($session_id) {
        ...
        if (!$redis) {
            ...
            $message = $redis_enabled
                ? 'Redis connection failed while validating the session.'
                : 'Session validation requires Redis. Use the one-time magic link or enable Redis in Live Events settings.';
        ...
        if ($use_pipeline) {
            ...
            $pipe->get("jwt:{$session_id}"); // Try to get JWT data too
        ...
        if (!$session_data) {
            $result = array('valid' => false, 'error' => 'Session not found');
            self::$memory_cache[$memory_key] = array_merge($result, array('_cached_at' => time()));
            return $result;
        }
        ...
        if (time() > $session['expires_at']) {
            if ($use_pipeline && $pipe) {
                $pipe->del("session:{$session_id}");
                $pipe->del("jti_session:{$session['jti']}");
                $pipe->execute();
            } else {
        ...
        $jwt_validation = $this->validate_jwt_direct_by_jti($session['jti']);
        if (!$jwt_validation['valid']) {
            $this->revoke_session($session_id);
            $result = array('valid' => false, 'error' => 'JWT invalid: ' . $jwt_validation['error']);
            self::$memory_cache[$memory_key] = array_merge($result, array('_cached_at' => time()));
            return $result;
        }
        
        // Store successful validation for quick access on event page
    ...
    public function revoke_session($session_id) {
        ...
        if ($session_data) {
            ...
            if ($session) {
                ...
                $redis->setex("session:{$session_id}", 24 * 60 * 60, json_encode($session));
                
                // Clean up related keys
                $redis->del("jti_session:{$session['jti']}");
                $email_hash = hash('sha256', $session['email']);
                $redis->del("active_session:{$session['event_id']}:{$email_hash}");
    ...
    public function revoke_all_sessions_for_event($event_id, $email) {
        ...
        $active_session_id = $redis->get("active_session:{$event_id}:{$email}");
        if ($active_session_id) {
            return $this->revoke_session($active_session_id);
        }
    ...
    private function validate_jwt_direct_by_jti($jti) {
        ...
        if (!$redis) {
            ...
            $redis_enabled = !empty($settings['use_redis']);
            $message = $redis_enabled
                ? 'Redis connection failed while validating the JWT.'
                : 'JWT session validation requires Redis. Enable Redis in Live Events settings or use the direct token link.';
            $result = array('valid' => false, 'error' => $message);
            self::$memory_cache[$memory_key] = array_merge($result, array('_cached_at' => time()));
            return $result;
        }
    ...
    public function render_event_ticket_block($attributes) {
        ...
        ob_start();
        include LEM_PLUGIN_DIR . 'templates/event-ticket-block.php';
        return ob_get_clean();
    }
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            ...
            'Restrictions',
            'manage_options',
            'live-event-manager-restrictions',
    ...
    public function render_stream_setup_page() {
        include LEM_PLUGIN_DIR . 'templates/stream-setup-page.php';
    }
    ...
    private function get_mux_api_credentials() {
        ...
        $this->debug_log('Mux API credentials check', array(
            'token_id_exists' => !empty($token_id),
            'token_secret_exists' => !empty($token_secret),
            'token_id_length' => strlen($token_id),
            'token_secret_length' => strlen($token_secret)
        ));
        ...
        $url = 'https://api.mux.com/video/v1/playback-restrictions';
                    ...
                    'auth_header' => 'Basic ' . base64_encode($credentials['token_id'] . ':' . $credentials['token_secret'])
                ));
    ...
    public function generate_jwt($email, $event_id, $payment_id = null, $is_refresh = false) {
        ...
        // Calculate expiration based on admin settings
        if ($is_refresh) {
            // Short-lived JWT for refresh (configurable minutes)
            $refresh_duration_minutes = $settings['jwt_refresh_duration_minutes'] ?? 15;
            $exp = time() + ($refresh_duration_minutes * 60);
            $this->debug_log('JWT refresh expiration calculation', array(
                'refresh_duration_minutes' => $refresh_duration_minutes,
                'exp' => $exp,
                'exp_human' => date('Y-m-d H:i:s', $exp)
            ));
        } else {
            // Initial JWT uses admin-configured hours
            $jwt_expiration_hours = $settings['jwt_expiration_hours'] ?? 24;
            $exp = time() + ($jwt_expiration_hours * 3600);
            $this->debug_log('JWT initial expiration calculation', array(
        ...
        $session_id = $this->create_session($random_jti, $event_id, $email);
        
        // Get device identifier using abstracted service
        $device_identifier = $this->device_service->getDeviceIdentifier(array(
            'session_id' => $session_id
        ));
        
        // Determine what identifier to use based on device settings
        ...
        switch ($identifier_type) {
            case 'session_based':
                ...
                break;
            case 'ip_address':
                $identifier_value = $device_identifier['metadata']['ip'] ?? '0.0.0.0';
                break;
            case 'fingerprint':
                $identifier_value = $device_identifier['identifier'] ?? 'unknown';
                break;
            case 'custom_token':
                $identifier_value = $device_identifier['identifier'] ?? 'unknown';
                break;
            case 'hybrid':
                $identifier_value = $session_id; // Use session ID as primary in hybrid
                break;
            default:
                $identifier_value = $session_id; // Default to session-based
        }
        
        // Create hash JTI for Redis lookup (backward compatibility)
        $ip = $device_identifier['metadata']['ip'] ?? '0.0.0.0';
        $hash_jti = hash('sha256', $email . '|' . $ip . '|' . $event->playback_id);
        
        // Use calculated expiration time
        $exp_int = (int)$exp;
        ...
        $payload = array(
            ...
            'custom' => array(
                ...
                'event_id' => $event_id,
                'ip' => $ip // Keep for backward compatibility but not used for validation
            )
        );
        
        // Only add playback_restriction_id if it's not empty
        ...
        try {
            $this->debug_log('Generating Mux JWT', array(
            ...
            // Store JWT in database and Redis
            $this->store_jwt($random_jti, $hash_jti, $jwt, $email, $event_id, $payment_id, $ip, date('Y-m-d H:i:s', $exp));
            
            // Store complete JWT data in Redis for fast Cloudflare Worker lookups
            $jwt_redis_data = array(
                'jti' => $random_jti,
                'jwt_token' => $jwt, // Add JWT token for caching
                'playback_id' => $event->playback_id,
                ...
                'identifier_value' => $identifier_value,
                'session_id' => $session_id,
                'ip_address' => $ip, // Legacy field for backward compatibility
                'fingerprint' => $device_identifier['type'] === 'fingerprint' ? $device_identifier['identifier'] : null,
            ...
            $this->debug_log('JWT and session generated successfully', array(
                'jti' => $random_jti,
                'session_id' => $session_id,
                'event_id' => $event_id,
                'email' => $email,
    ...
    public function ajax_generate_jwt() {
        ...
        if ($result && is_array($result)) {
            ...
            $this->magic_link_service->send_magic_link_email($email, $result['jwt'], $event_id, $result['session_id']);
            
            if (!headers_sent() && !empty($result['session_id'])) {
                setcookie('lem_session_id', $result['session_id'], time() + DAY_IN_SECONDS, '/', '', is_ssl(), true);
                $_COOKIE['lem_session_id'] = $result['session_id'];
            }
            ...
            wp_send_json_success(array(
                ...
                'message' => 'Access granted! Check your email for the magic link.'
            ));
        } else {
    ...
    public function ajax_get_jwt_tokens() {
        ...
        $tokens = $wpdb->get_results("
            SELECT jti, email, event_id, ip_address, created_at, expires_at, jwt_token 
            FROM $table 
    ...
    public function ajax_create_stripe_session() {
        ...
        $this->debug_log('All settings keys', array_keys($settings));
        $this->debug_log('Test secret key field: ' . ($settings['stripe_test_secret_key'] ?? 'NOT SET'));
        $this->debug_log('Live secret key field: ' . ($settings['stripe_live_secret_key'] ?? 'NOT SET'));
        
        if ($stripe_mode === 'test') {
            $secret_key = $settings['stripe_test_secret_key'] ?? '';
            $this->debug_log('Using test mode, secret key length: ' . strlen($secret_key));
            $this->debug_log('Test secret key starts with: ' . substr($secret_key, 0, 7));
        } else {
            $secret_key = $settings['stripe_live_secret_key'] ?? '';
            $this->debug_log('Using live mode, secret key length: ' . strlen($secret_key));
            $this->debug_log('Live secret key starts with: ' . substr($secret_key, 0, 7));
        }
        ...
        try {
            ...
            $session = \Stripe\Checkout\Session::create([
                ...
                'cancel_url' => home_url('/?lem_cancel=1'),
                'metadata' => [
                    'event_id' => $event_id,
    ...
    public function ajax_regenerate_jwt() {
        ...
        // Generate new JWT
        $jwt_result = $this->generate_jwt($email, $event_id, $token->payment_id);
        
        if ($jwt_result && is_array($jwt_result)) {
            // Send magic link email
                    $this->magic_link_service->send_magic_link_email($email, $jwt_result['jwt'], $event_id, $jwt_result['session_id'] ?? null);
    ...
    public function ajax_test_redis_connection() {
        ...
        $use_redis_post = isset($_POST['use_redis']) ? intval($_POST['use_redis']) : null;
        $use_redis = $use_redis_post !== null ? $use_redis_post : (isset($settings['use_redis']) ? intval($settings['use_redis']) : 0);

        if ($use_redis !== 1) {
            wp_send_json_error('Redis is disabled in plugin settings. Enable it in Live Events > Settings > Redis Configuration.');
        }

        $host = isset($_POST['redis_host'])
            ? sanitize_text_field(wp_unslash($_POST['redis_host']))
            : ($settings['redis_host'] ?? '127.0.0.1');
        $port = isset($_POST['redis_port'])
            ? intval($_POST['redis_port'])
            : (isset($settings['redis_port']) ? intval($settings['redis_port']) : 6379);
        if ($port <= 0) {
            $port = 6379;
        }
        $password = isset($_POST['redis_password'])
            ? sanitize_text_field(wp_unslash($_POST['redis_password']))
            : ($settings['redis_password'] ?? '');
        $database = isset($_POST['redis_database'])
            ? intval($_POST['redis_database'])
            : (isset($settings['redis_database']) ? intval($settings['redis_database']) : 0);
        ...
        try {
            $this->debug_log('Redis test - starting connection test', array(
                'host' => $host,
                'port' => $port,
                'database' => $database
            ));
            
            $redis = $this->get_redis_connection($override_settings);
            if (!$redis) {
                $this->debug_log('Redis test - connection failed');
                if (!class_exists('Redis')) {
                    wp_send_json_error('Redis PHP extension is not installed. Please enable it on the server.');
                }
                wp_send_json_error('Redis connection failed: Could not establish connection. Check your Redis settings and ensure Redis server is running.');
            }
            $client_type = ($redis instanceof LEM_Object_Cache_Adapter) ? 'object_cache_adapter' : 'phpredis';
            $this->debug_log('Redis test - connection successful', array('client' => $client_type));
            ...
            // Test SET
            $set_result = $redis->set($test_key, $test_value, 60); // 60 second TTL
            if (!$set_result) {
                wp_send_json_error('Redis SET operation failed');
            }
            
            // Test GET
            $get_result = $redis->get($test_key);
            if ($get_result !== $test_value) {
                wp_send_json_error('Redis GET operation failed. Expected: ' . $test_value . ', Got: ' . $get_result);
            }
            
            // Test DELETE
            $del_result = $redis->del($test_key);
            if ($del_result !== 1) {
                wp_send_json_error('Redis DEL operation failed');
            }
            ...
            $current_db = method_exists($redis, 'getDBNum') ? $redis->getDBNum() : $database;
            ...
            wp_send_json_success(array(
                ...
                'host' => $host,
                'port' => $port,
                'client_type' => ($client_type === 'object_cache_adapter') ? 'WordPress Object Cache' : 'PHP Redis extension'
            ));
            
        } catch (Exception $e) {
            wp_send_json_error('Redis connection failed: ' . $e->getMessage());
        }
    }
    
    // Get database logs (last 10 JWT entries)
    ...
    // Redis helper methods with connection pooling
    public function get_redis_connection($override_settings = array()) {
        ...
        $use_redis = isset($settings['use_redis']) ? intval($settings['use_redis']) : 0;
        if ($use_redis !== 1) {
            return false;
        }

        // Reuse existing connection if available and valid (connection pooling)
        if (self::$redis_instance !== null && self::$redis_instance !== false) {
            try {
                // Quick ping to verify connection is still alive
                if (self::$redis_instance instanceof LEM_Object_Cache_Adapter) {
                    // Object cache adapter doesn't need ping
                    return self::$redis_instance;
                } elseif (method_exists(self::$redis_instance, 'ping')) {
                    if (self::$redis_instance->ping()) {
                        return self::$redis_instance;
                    }
                } else {
                    // Assume connection is good if ping method doesn't exist
                    return self::$redis_instance;
                }
            } catch (Exception $e) {
                // Connection failed, reset and create new one
                self::$redis_instance = null;
            }
        }

        // Preferred: native Redis extension
        if (class_exists('Redis')) {
            try {
                $redis = new Redis();
                // Optimized timeouts for Upstash (serverless)
                $redis->setOption(Redis::OPT_READ_TIMEOUT, 2);
                $redis->setOption(Redis::OPT_TCP_KEEPALIVE, 1);
                // Enable connection persistence for better performance
                $redis->setOption(Redis::OPT_SCAN, Redis::SCAN_RETRY);

                $host = $settings['redis_host'] ?? '127.0.0.1';
                $port = isset($settings['redis_port']) ? intval($settings['redis_port']) : 6379;
                $timeout = isset($settings['redis_timeout']) ? floatval($settings['redis_timeout']) : 2;
                if ($timeout <= 0) {
                    $timeout = 2; // Reduced for Upstash
                }

                // Use persistent connection if available (better for Upstash)
                $persistent_id = 'lem_redis_' . md5($host . $port);
                $connected = $redis->pconnect($host, $port, $timeout, $persistent_id);
                ...
                $database = isset($settings['redis_database']) ? intval($settings['redis_database']) : 0;
                $selected_db = $this->select_redis_database($redis, $database);
                ...
                $this->debug_log('Redis connection error', array('error' => $e->getMessage()));
                return false;
            }
        }

        // Fallback: WordPress object cache adapter (e.g., Predis plugin)
        if ($this->is_object_cache_available()) {
            $adapter = $this->get_object_cache_adapter();
            self::$redis_instance = $adapter;
            return $adapter;
        }
    ...
    private function select_redis_database($redis, $preferred_db = 0) {
        if ($redis instanceof LEM_Object_Cache_Adapter) {
            return 0;
        }
        try {
            if ($redis->select($preferred_db)) {
                return $preferred_db;
            }
    ...
    private function get_object_cache_adapter() {
        return new LEM_Object_Cache_Adapter();
    }
        ...
        if ($redis) {
            ...
            if ($jwt_data) {
                ...
                if ($jwt_info) {
                    ...
                    $jwt_info['revoked_at'] = date('Y-m-d H:i:s');
            ...
            if (!empty($event_id)) {
                ...
                $redis->del("active_session:{$event_id}:{$email_hash}");
     ...
     */
    public function list_live_streams($request) {
        $credentials = $this->get_mux_api_credentials();
        if (!$credentials) {
            return new WP_Error('mux_not_configured', 'Mux API credentials not configured', array('status' => 500));
        }
        ...
        // Call Mux API to list live streams
        ...
        if (!$data || isset($data['error'])) {
            return new WP_Error('mux_api_error', 'Failed to fetch live streams', array('status' => 500));
        }
        ...
        // Cache for 5 minutes
        if ($redis) {
            $redis->setex($cache_key, 300, json_encode($result));
        }
    ...
    private function validate_jwt_by_hash($email, $ip, $playback_id) {
        $this->debug_log('Validating JWT by hash', array('email' => $email, 'ip' => $ip, 'playback_id' => $playback_id));
    ...
    public function handle_stripe_webhook() {
        ...
        try {
            ...
            if (!class_exists('\Stripe\Webhook')) {
                ...
                exit('Stripe library not available');
            }
        ...
        if ($event->type === 'checkout.session.completed') {
            ...
            if ($event_id && $email) {
                $this->debug_log('Processing payment for event: ' . $event_id . ', email: ' . $email);
                ...
                if ($existing_token) {
                    ...
                    // Generate JWT (this is the ONLY place JWTs should be generated for paid events)
    ...
    private function create_session_for_magic_token($email, $event_id) {
        return $this->create_session(bin2hex(random_bytes(16)), $event_id, $email);
    }
    ...
    public function get_event_access_state($event_id) {
        ...
        if (!empty($session_id)) {
            ...
            if (!empty($session_validation['valid'])) {
                ...
                $state['error_message'] = $session_validation['error'] ?? __('Session expired. Request a new link to continue.', 'live-event-manager');
                $this->clear_session_cookie();
            }
        }
    ...
    // Get JWT token for a session (Redis-optimized with in-memory cache)
    public function get_jwt_for_session($session_id) {
        ...
        if ($use_pipeline) {
            ...
            $pipe->get("session:{$session_id}");
            $results = $pipe->execute();
            $session_data_json = $results[0] ?? false;
        } else {
        ...
        // Check if JWT is revoked BEFORE retrieving it
        $is_revoked = false;
        if ($use_pipeline && $pipe) {
            $pipe->exists("revoked:{$jti}");
            $pipe->get("jwt_token:{$jti}");
            $results = $pipe->execute();
            $is_revoked = $results[0] ?? false;
            $jwt_token = $results[1] ?? false;
        } else {
            $is_revoked = $redis->exists("revoked:{$jti}");
            $jwt_token = $redis->get("jwt_token:{$jti}");
        }
        
        // If revoked, don't return JWT
        if ($is_revoked) {
            $this->debug_log('JWT revoked, not returning token', array('jti' => $jti, 'session_id' => $session_id));
            return null;
        }
        
        if ($jwt_token) {
            // Cache in memory
            self::$memory_cache[$memory_key] = $jwt_token;
            return $jwt_token;
        }
        ...
        if ($token_record) {
            // Cache JWT in Redis for future requests (24-hour TTL)
            $redis->setex("jwt_token:{$jti}", 24 * 60 * 60, $token_record->jwt_token);
            
            // Cache in memory
            self::$memory_cache[$memory_key] = $token_record->jwt_token;
    ...
    // Get IP subnet for device fingerprinting
    ...
    // AJAX handler for JWT refresh
    public function ajax_refresh_jwt() {
        try {
            $this->debug_log('JWT refresh request started');
            ...
            $session_id = $_COOKIE['lem_session_id'] ?? '';
            if (empty($session_id)) {
                $this->debug_log('JWT refresh failed - no session ID in cookie');
                wp_send_json_error('No active session found');
                return;
            }
            
            // Validate session
            $session_validation = $this->validate_session($session_id);
            if (!$session_validation['valid']) {
                $this->debug_log('JWT refresh failed - invalid session', array(
                    'session_id' => $session_id,
                    'error' => $session_validation['error'] ?? 'unknown'
                ));
                wp_send_json_error('Invalid session: ' . ($session_validation['error'] ?? 'unknown'));
                return;
            }
            
            $session = $session_validation['session'];
            $event_data = $session_validation['event'];
            
            // Generate new short-lived JWT
            $refresh_result = $this->generate_jwt($session['email'], $session['event_id'], null, true);
            
            if (!$refresh_result || !isset($refresh_result['jwt'])) {
                $this->debug_log('JWT refresh failed - could not generate new JWT');
                wp_send_json_error('Failed to generate new JWT');
                return;
            }
            
            $this->debug_log('JWT refresh successful', array(
                'session_id' => $session_id,
                'event_id' => $session['event_id'],
                'email' => $session['email'],
                'new_jti' => $refresh_result['jti'] ?? 'unknown'
            ));
            
            wp_send_json_success(array(
                'jwt' => $refresh_result['jwt'],
                'expires_at' => $refresh_result['expires_at'] ?? null,
                'refresh_duration_minutes' => $settings['jwt_refresh_duration_minutes'] ?? 15,
                'refresh_in_minutes' => ($settings['jwt_refresh_duration_minutes'] ?? 15) - 1 // Refresh 1 minute before expiry
            ));
            
        } catch (Exception $e) {
            $this->debug_log('JWT refresh exception', array(
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ));
            wp_send_json_error('JWT refresh failed: ' . $e->getMessage());
        }
    }
