...
class DeviceIdentificationService {
    ...
    public function updateIdentificationMethod($method) {
        ...
        $allowed_methods = array('session_based', 'ip_address', 'fingerprint', 'custom_token', 'hybrid');
    ...
    public function updateSettings($new_settings) {
        ...
        $allowed_keys = array(
            'identification_method', 'ip_fallback_enabled', 'session_fallback_enabled',
            'fingerprint_enabled', 'custom_token_enabled', 'strict_mode',
        );
        $filtered = array_intersect_key($new_settings, array_flip($allowed_keys));
        $this->settings = array_merge($this->settings, $filtered);
        update_option('lem_device_settings', $this->settings);
    }
}